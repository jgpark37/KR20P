...
{	
		...
		if (!sTLcd_Info.flag) { // first
			...
			if ((sTLcd_Info.buf[sTLcd_Info.buf_cnt-1] == '!' && // down mark start
				...
				sTLcd_Info.buf[sTLcd_Info.buf_cnt] = sTLcd_q.buf[sTLcd_q.tail];
                                sTLcd_Info.buf_cnt++;
				if (sTLcd_Info.buf_cnt >= TLCD_CMD_BUF_SIZE) sTLcd_Info.buf_cnt = 0;
				
				//exception
				if (sTLcd_Info.buf_cnt >= 3 && sTLcd_Info.buf[0] != 't') {
	...
	case '>':
	case ')':
		if (sTLcd_Info.buf[0] == '<' &&  sTLcd_q.buf[sTLcd_q.tail] == '>') { // down mark			
			...
			if (sTLcd_Info.flag & TLCD_FLAG_PREDOWN) {
                ...
                sTLcd_Info.point_up.x = 0;
                sTLcd_Info.point_up.y = 0;
			}
			else {
				sTLcd_Info.buf_cnt = 0;
