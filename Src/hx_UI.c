...
struct _tagPATIENT_INFO_WND{ //ȯ������ â�� ���� ����
	//uint16_t id;
...
struct _tagPATIENT_INFO_WND2{ //ȯ������ â�� ���� ����
#ifdef PI_LOAD_V2
	uint32_t id;
#else
	uint16_t id;
...
typedef __packed struct _tagBASETUP_INFO{    //�������� â�� ���� ����
	short exAngle, flAngle;					//Extension(????) ????(flection) 
	//char mode;  //������ ���� ��� ����
}BASETUP_INFO;
BASETUP_INFO AngleWnd, AngleWndOld;

typedef __packed struct _tagBASETUP2_INFO{    //�ӵ�/�ð� ���� â�� ���� ����
	uint16_t speed;     //�ӵ� //smart use
	uint16_t time;      //�ð� //smart use
}BASETUP2_INFO;
BASETUP2_INFO SpdTmWnd, SpdTmWndOld;

uint16_t SpdTmWnd_bk_speed; //pjg++170530 �ӽ÷� ����.�ӵ��� 1�ΰ�� �ʹ� �ʾ �ӽ������� ����

struct {    //���� â�� ���� ����
	uint8_t dir;        //���� �̵�����
	uint8_t play;       //���� ����(play, playing, pause, stop
	uint16_t repeat;    //���� ������ Ƚ��
	uint16_t tick;      //���� �����ϴ� Ÿ�̸� ����
	uint32_t time;      //���� �����ϴ� �ð�
	int angle;          //���� �����ǰ� �ִ� ���� ����
...
int UI_SavePIToEEPROM(uint16_t id, uint8_t *buf)
{
	...
	//for (i = 0; i < 100000; i++);  //������ EEPROM ������ �ȵ� 
	//for (i = 0; i < 150000; i++);  //������ EEPROM ������ �ȵ� 
	return 1;
}
...
void UI_InitSetupVariable(void)    //�ý��� �ʱ�ȭ �ÿ� 1�� �ϸ�  EEPROM ���� �ҷ������� ������
{
...
void UI_InitVariable(void) // �÷��� ���Ʒ� �Ϸ� �� �ٽ� ����    ����  
{
	//int i;
...
//          Total_Counter = 0;
	...
	RunWnd.dir = 0;                     //���� ����
//	RunWnd.angle = AngleWnd.exAngle;    //������ �������� ����
	RunWnd.repeat = 0;                    //���� �ݺ�ȸ�� �ʱ�ȭ
//       RunWnd.angle = 0;//Current_Angle - 5;	//pjg<>171222
	RunWnd.tick = 0;                    //���� �����ð� ƽ(�ð�)��
	RunWnd.time = SpdTmWnd.time;        //������ �ð�(�� ����
...
void UI_Run_OnBnClickedBtnPlayStart(void)
{
	...
	//else
	{
		...
		//App_SetTimer(1, 150000); // �ʹ� ������ ���� : max speed(150000)
		App_SetTimer(TIMER_ID_2, 30);//00000); // �ʹ� ������ ���� : max speed
...
void UI_Run_OnBnClickedBtnPlayStart(void)
{
	...
	if (Setup2.sndGuid2== BST_CHECKED)
		APP_SendMessage(hParent, WM_SOUND, 0, SNDID_OPERATION_START);
	//App_SetTimer(1, 150000); // �ʹ� ������ ���� : max speed(150000)
	App_SetTimer(TIMER_ID_2, 200); // �ʹ� ������ ���� : max speed
	App_SetUIProcess(UI_Run_Process); //pjg++170529
	UI_LED_Control(LM_RUN);
	Total_Counter = PInfoWnd2.pi.totalRepeat;
	Timer_sec = 0; //pjg++170608 buf fix: complete right now
...
//	APP_SendMessage(hParent, WM_PAINT, 0, (LPARAM)pBtnInfo[RID_RN_BTN_PLAYING*2]);
	...
	APP_SendMessage(hParent, WM_SOUND, 0, SNDID_OPERATION_START);
	//App_SetTimer(1, 150000); // �ʹ� ������ ���� : max speed(150000)
	App_SetTimer(TIMER_ID_6, 50);//00000); // �ʹ� ������ ���� : max speed
...
void UI_Timer(uint16_t nIDEvent)
{
	...
	switch (nIDEvent) {
	case TIMER_ID_0:    //init �ý��� �ʱ�ȭ ���α׷����� ǥ��
		//UI_SystemInit_Timer();
		break;
	case TIMER_ID_1:    //loading Ȩ������ �̵��Ҷ� ǥ��
		UI_Loading_Timer(0);
		break;
	case TIMER_ID_2:    //playing �����Ҷ� ����
		if (RunWnd.time > 0){
			if(RunWnd.play != UI_RUN_MODE_HOME){
				if(Timer_sec >= 6){	//1min
					RunWnd.time--;
